...
class GameField(object):
    ...
    def move(self, direction):
        def move_row_left(row):
            ...
            # merges 2 tiles if they are equal and multiplies the value by itself
            # of the new tile. Them upates the score accordingly
            def merge(row):
                pair = False
                new_row = []
                for i in range(len(row)):
                    if pair:
                        ...
                        # if same value will append the 2 tiles
...
def main(stdscr):
    ...
    def game():
        # based on response will act appropriately (user says
        # restart it will restart)
        game_field.draw(stdscr)
        action = get_user_action(stdscr)
        if action == 'Restart':
            return 'Init'
